...
final class PhoneViewModel: ObservableObject {
    ...
    func observeRequests() async {
        ...
        for await request in requests {
            if Task.isCancelled { break }
            let reply = BridgeMessage(text: "Reply from iPhone", origin: "iPhone", id: request.payload.id)
    ...
    func sendPrimaryRequest() async {
        await sendRequest(text: "Hello from the phone app", timeout: 10)
    }

    func sendReplyRequest() async {
        await sendRequest(text: "Requesting a reply from the watch", timeout: 2)
    }

    private func sendRequest(text: String, timeout: Int) async {
        let requestId = UUID()
        let message = BridgeMessage(text: text, origin: "iPhone", id: requestId)
